  ...
  async create(createTransactionDto: CreateTransactionDto): Promise<Transaction> {
    return this.transactionRepository.manager.transaction(
      async (manager: EntityManager) => {
        const fromAccount = await manager.findOneBy(Account, { 
          id: createTransactionDto.fromAccountId 
        });
        const toAccount = await manager.findOneBy(Account, { 
        ...
        if (fromAccount.currency !== toAccount.currency) {
          throw new BadRequestException("Currency mismatch: both accounts must have the same currency");
        }
        ...
        await manager.update(Account, fromAccount.id, {
          balance: Number(fromAccount.balance) - Number(createTransactionDto.amount),
        });

        await manager.update(Account, toAccount.id, {
          balance: Number(toAccount.balance) + Number(createTransactionDto.amount),
        });
